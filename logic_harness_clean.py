...
if __name__ == "__main__":
    ...
    boeing  = ("boeing_events_multisource.csv",            "BOEING 737 MAX (2011-2019)")
    uhc     = ("unitedhealth_events_extracted_FINAL.csv",  "UNITEDHEALTHCARE (2017-2024)")
    wells   = ("wellsfargo_events_extracted.csv",          "WELLS FARGO (2009-2020)")

    if not args or "--all" in args:
        to_run = [boeing, uhc, wells]
    else:
        lookup = {"boeing": boeing, "uhc": uhc, "wells": wells}
        to_run = [lookup[a] for a in args if a in lookup]

    print("\nRUNNING SAME HARNESS ON ALL DATASETS — NO PARAMETER CHANGES BETWEEN RUNS")
    results = [r for r in [run(path, label) for path, label in to_run] if r is not None]

    if len(results) >= 2:
        labels = [label for _, label in to_run]
        print(f"\n{'='*95}")
        print("CROSS-DATASET COMPARISON")
        print(f"{'='*95}")
        for i, (fac, conf, state, thresh) in enumerate(results):
            print(f"\n{labels[i]}")
